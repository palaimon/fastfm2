...
#ifndef FASTFM_CORE2_FASTFM_TESTS_API_TESTS_FIXTURE_H_
#define FASTFM_CORE2_FASTFM_TESTS_API_TESTS_FIXTURE_H_
...
class FMExample {
 public:
  FMExample() {
    ...
    pd_ = fastfm::DataFactory().get();
//    fastfm::Data::Impl* data = fastfm::Internal::get_impl(pd_);
    pm_ = fastfm::ModelFactory().get();
    Model::Impl* model = fastfm::Internal::get_impl(pm_);
    ps_ = new fastfm::Settings();
    Settings::Impl* settings = fastfm::Internal::get_impl(ps_);
    // Allocate space for the predictions.
    Vector y_pred = Vector::Zero(x_.rows());
    pd_ = fastfm::DataFactory(x_, &y_pred).get();
 ...
 protected:
  ...
  Vector w1_;
  double w0_;
  fastfm::ModelParam coef_;
  fastfm::SolverSettings settings_;
  Data* pd_;
  Model* pm_;
  Settings* ps_;
//  Vector y_naive;
...
class IFMExample {
 protected:
  IFMExample() {
    ...
    // TODO(Immanuel): Fix the all zero feature
    // X_naive = | 1 0 0 | 1 2 |
    //           | 1 0 0 | 3 0 |
    //           | 1 0 0 | 0 6 |
    //           | 2 0 4 | 1 2 |
  ...
  fastfm::ModelParam coef_naive;
  fastfm::SolverSettings settings_naive_;

  const std::vector<double> v_empty_;
};
#endif  // !EXTERNAL_RELEASE
#endif  // FASTFM_CORE2_FASTFM_TESTS_API_TESTS_FIXTURE_H_
