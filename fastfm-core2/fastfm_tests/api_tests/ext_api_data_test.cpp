...
using Matrix = Eigen::Matrix<double,
                             ...
                             Eigen::Dynamic,
                             Eigen::RowMajor>;
using fastfm::Internal;

TEST_CASE("Check default settings", "[check_default_settings]") {
  Settings* s = new Settings();
  REQUIRE(Approx(fastfm::Internal::get_impl(s)->settings_.init_var_w2) == 0.1);
...
TEST_CASE("Add parameter to Model", "[add_parameter]") {
  ...
  REQUIRE(Approx(Internal::get_impl(m)->coef_->getMapValue("one").coeff(0))
      == 1.1);
  REQUIRE(Approx(Internal::get_impl(m)->coef_->getMapValue("ten").coeff(0))
              == 10.2);
  REQUIRE(Approx(Internal::get_impl(m)->coef_->getMapValue("hundred").coeff(0))
              == 100.3);

  REQUIRE(
      Approx(Internal::get_impl(m)->coef_->getMapValue("one").size()) == 1);
  REQUIRE(
      Approx(Internal::get_impl(m)->coef_->getMapValue("ten").size()) == 1);
  REQUIRE(
      Approx(Internal::get_impl(m)->coef_->getMapValue("hundred").size()) == 1);
...
TEST_CASE("Default Settings", "[default_settings]") {
  Settings* s = new Settings();
  fastfm::SolverSettings default_;
  REQUIRE(Approx(Internal::get_impl(s)->settings_.init_var_w2)
              == default_.init_var_w2);
...
TEST_CASE("Data, column major matrix", "[add_design_matrix_col_major]") {
  ...
  REQUIRE(Approx(Internal::get_impl(d)->get_design_matrix_col_major().norm())
              == x.norm());
  delete d;
}
...
TEST_CASE("Data, row major matrix", "[add_design_matrix_row_major]") {
  ...
  REQUIRE(Approx(Internal::get_impl(d)->get_design_matrix_row_major().norm())
              == x.norm());
  delete d;
}
