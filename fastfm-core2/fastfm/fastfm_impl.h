...
#ifndef FASTFM_CORE2_FASTFM_FASTFM_IMPL_H_
#define FASTFM_CORE2_FASTFM_FASTFM_IMPL_H_
...
class ModelParam {
 ...
 public:
  ModelParam()
      : w0map(NULL), w1map(NULL), w2map(NULL), w3map(NULL), mapValues(NULL) {
  }

  ModelParam(const SolverSettings& settings, const int n_features)
      : w0map(NULL), w1map(NULL),
...

    {
      ...
      std::normal_distribution<double>
          distribution(0, std::sqrt(settings.init_var_w2));
...

    {
      ...
      std::normal_distribution<double>
          distribution(0, std::sqrt(settings.init_var_w3));
  ...
  VectorRef getw1block(const unsigned int rowstart, const unsigned int rownum) {
    if (w1map != NULL) {
      return Eigen::Map<Vector, 0,
                        outerStride>(&(*w1map)(rowstart, 0),
                                     rownum,
                                     outerStride(w1map->outerStride()));
    } else {
      return
          Eigen::Map<Vector, 0,
                     outerStride>(&w1(rowstart, 0),
                                  rownum,
                                  outerStride(w1.outerStride()));
    }
  }

  constVectorRef getw1block(const unsigned int rowstart,
                            const unsigned int rownum) const {
    if (w1map != NULL)
      return Eigen::Map<const Vector, 0,
                        outerStride>(&(*w1map)(rowstart, 0),
                                     rownum,
                                     outerStride(w1map->outerStride()));
    return Eigen::Map<const Vector, 0,
  ...
  MatrixRef getw2block(const unsigned int rowstart,
                       const unsigned int colstart,
                       const unsigned int rownum,
                       const unsigned int colnum) {
    if (w2map != NULL) {
      if ((rownum == 0 && colnum == 0)
          || rowstart + 1 > w2map->rows()
          || colstart + 1 > w2map->cols())
        return Eigen::Map<Matrix, 0,
                          outerStride>(w2map->data(),
                                       0, 0,
                                       outerStride(0));
      return Eigen::Map<Matrix, 0,
                        outerStride>(&(*w2map)(rowstart, colstart),
                                     rownum, colnum,
                                     outerStride(w2map->outerStride()));
    }
    return w2.block(rowstart, colstart, rownum, colnum);
  }

  constMatrixRef getw2block(const unsigned int rowstart,
                            const unsigned int colstart,
                            const unsigned int rownum,
                            const unsigned int colnum) const {
    if (w2map != NULL) {
      if ((rownum == 0 && colnum == 0) || rowstart + 1 > w2map->rows()
          || colstart + 1 > w2map->cols())
        return Eigen::Map<const Matrix, 0,
                          outerStride>(w2map->data(),
                                       0, 0,
                                       outerStride(0));
      return Eigen::Map<const Matrix, 0,
...
class Data::Impl {
 ...
 public:
  ...
  void wrap_train_target_memory(double* data, int n_samples) {
    // C++ placement operator.
    // Does not allocate memory, but runs the constructor of the class
    // on the memory pointer provided.
  ...
  int wrap_design_matrix_col_major(const std::string& name,
                                   double* data,
                                   int n_samples,
                                   int n_features,
                                   int nnz,
                                   int* outer,
                                   int* inner) {
    auto res = x_.emplace(name,
                          Eigen::Map<SpMat>(n_samples,
                                            n_features,
                                            nnz,
                                            outer,
                                            inner,
  ...
  int wrap_design_matrix_row_major(const std::string& name,
                                   double* data,
                                   int n_samples,
                                   int n_features,
                                   int nnz,
                                   int* outer,
                                   int* inner) {
    auto res = x_row_.emplace(name,
                              Eigen::Map<RowSpMat>(n_samples,
                                                   n_features,
                                                   nnz,
                                                   outer,
                                                   inner,
...
// NOLINTNEXTLINE
        ...
        std::istringstream(item.second) >> std::boolalpha
                                        >> settings_.zero_order;
      } else if (item.first == "first_order") {
        std::istringstream(item.second) >> std::boolalpha
                                        >> settings_.first_order;
      }
// NOLINTNEXTLINE
...
#endif  // FASTFM_CORE2_FASTFM_FASTFM_IMPL_H_
