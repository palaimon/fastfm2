...
void FitSquareLoss(constSpMatRef x, constVectorRef y, constVectorRef cost,
    ...
    for (; i < settings.iter; ++i) {
        ...
        if (third_order) {
            Predict(x, coef->getw3(), coef->getw2(), coef->getw1(), coef->getw0(), err);
        } else {
            Predict(x, coef->getw2(), coef->getw1(), coef->getw0(), err);
        }
        ...
        for (int j = 0; settings.first_order && j < n_features; ++j) {
            ...
            // TODO don't recalculate che it's constant
        ...
        for (int f = 0; second_order && f < coef->getw2().rows(); ++f) {
            ...
            for (int j = 0; j < n_features; ++j) {
                ...
                SecondOrderStats(f, j, weight, x, coef->getw2(), err, q_cache, &chsqr, &che);
                double w_new = 0;
                if (is_mcmc) {
                    #if !EXTERNAL_RELEASE
                ...
                SecondOrderErrAndQcacheUpdate(f, j, coef->getw2(), w_old, x, &err, &q_cache);
            }
        }
        ...
        for (int f = 0; third_order && f < coef->getw3().rows(); ++f) {
            ...
            for (int j = 0; j < n_features; ++j) {
                ...
                ThirdOrderStats(f, j, weight, x, coef->getw3(), err, q_cache, q2_cache, &chsqr, &che);
                const double w_new = (che + w_old * chsqr) / (chsqr + settings.l2_reg_w3);
