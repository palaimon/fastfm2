...
Settings::Settings(const std::map<std::string, std::string>& settings_map) : mImpl(new Settings::Impl(settings_map)) {
...
void Model::add_matrix(const std::string& name, double* data, size_t rows, size_t cols, bool rowMajor) {
    CHECK_GE(rows, 0);
    CHECK_GE(cols, 0);
...
void Model::add_scalar_map(const std::string& keys, double* values, size_t size) {
...
void Data::add_vector(const std::string& name, double* data, const size_t size) {
    if (name == "y_true") {
...
void Data::add_matrix(const std::string& name, double* data, size_t rows, size_t cols, bool rowMajor) {
        CHECK_GE(rows, 0);
        CHECK_GE(cols, 0);
    if (name == "y_rec") {
...
void Data::add_sparse_matrix(const std::string& name, double* data, size_t rows, size_t cols, int nnz,
                             int* outer, int* inner, bool col_major) {
    if (name == "x" || name == "x_c" || name == "x_i") {
        if (col_major) { mImpl->wrap_design_matrix_col_major(name, data, rows, cols, nnz, outer, inner); }
        else {
            mImpl->wrap_design_matrix_row_major(name, data, rows, cols, nnz, outer, inner);
        }
    } else {
        CHECK(false) << "Name: " << name << " is not supported";
    }
...
void fit(Settings* s, Model* m, Data* d, fit_callback_t cb, python_function_t python_func) {
    ...
    #ifdef CD
    if ((settings->settings_.solver == "cd" || settings->settings_.solver == "mcmc") &
        (settings->settings_.loss == "squared" || settings->settings_.loss == "logistic")) {
    ...
    CHECK(false) << "Solver: " << settings->settings_.solver << " is not supported";
}
