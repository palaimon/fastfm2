...
from fastFM.tests.test_base import no_als_classification_skip
...
def test_fm_regression_reg_w():
    X, y = get_small_data()

    fm = als.FMRegression(n_iter=10, l2_reg_w=1, l2_reg_V=1, rank=0,
                          random_state=123)
    fm.fit(X, y)
    y_pred_reg1 = fm.predict(X)

    fm = als.FMRegression(n_iter=10, l2_reg_w=100, l2_reg_V=1, rank=0,
                          random_state=123)
    fm.fit(X, y)
...
def test_fm_regression_reg_V():
    ...
    fm = als.FMRegression(n_iter=10, l2_reg_w=1, l2_reg_V=1, rank=4,
                          random_state=123)
    fm.fit(X, y)
    y_pred_reg1 = fm.predict(X)

    fm = als.FMRegression(n_iter=10, l2_reg_w=1, l2_reg_V=100, rank=4,
                          random_state=123)
    fm.fit(X, y)
    y_pred_reg100 = fm.predict(X)
