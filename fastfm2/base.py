...
def _init_parameter(fm, n_features, w_rand=False):
    generator = check_random_state(fm.random_state)
    w0 = np.zeros(1, dtype=np.float64)
    w = np.zeros(n_features, dtype=np.float64)
    if w_rand:
        w = generator.normal(loc=0.0, scale=fm.init_stdev,
                             size=(n_features))
...
def _settings_factory(fm):
    ...
    if hasattr(fm, 'n_epoch'):
        settings_dict['n_epoch'] = fm.n_epoch

    # temporary stringificaton (see: fastFM-130)
    return {k: str(v).lower() for k, v in settings_dict.items()}
